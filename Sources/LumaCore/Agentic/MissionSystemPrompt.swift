...
public enum MissionSystemPrompt {
    ...
    public static let codeStyle = """
        ...
        Custom instruments export `instrument: CustomInstrument`. Place `export const instrument` at the top of the file (after imports and any file-level constants) per newspaper order — its `create` method's helpers follow below in call order. Before authoring, call `read_custom_instrument_template()` for the canonical skeleton and `read_custom_instrument_typings(def_id)` for the exact ambient + per-def TypeScript types the editor enforces; those are authoritative for the API surface (ctx, widgets, features, restored, onAction).
        ...
        - **`NativePointer.read*String(size)` is not bugged.** Passing a size means *"this string is not NUL-terminated; read exactly this many bytes"* (for `readCString`/`readUtf8String`/`readAnsiString`) or *"this many code units"* (for `readUtf16String`). Omit the size argument when you want NUL-terminated reading. `readCString` escapes anything it can't decode as valid UTF-8 (including embedded NULs) into `\\x00`-style sequences, so a return value that looks like it "contains nulls" actually means the buffer wasn't the NUL-terminated UTF-8 string you assumed — drop the size or use a different reader, don't parse manually.
