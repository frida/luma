...
struct VirtualizationLinuxImage {
    ...
    init(request: VirtualMachineLaunchRequest) throws {
        ...
        let symbols = request.text(VirtualizationParameter.symbols) ?? ""
        self.symbols = symbols.isEmpty ? nil : URL(fileURLWithPath: symbols)
        ...
        self.identity = request.storageDirectory.appendingPathComponent("MachineIdentifier")
    }
    ...
    func makeConfiguration(memory: Int, processors: Int) throws -> VZVirtualMachineConfiguration {
        ...
        configuration.platform = makePlatform()
    ...
    /// Saved state names the machine it came from, so the guest is given the
    /// same identity every boot rather than a fresh one.
    private func makePlatform() -> VZGenericPlatformConfiguration {
        let platform = VZGenericPlatformConfiguration()

        if let stored = try? Data(contentsOf: identity), let identifier = VZGenericMachineIdentifier(dataRepresentation: stored) {
            platform.machineIdentifier = identifier
        } else {
            try? platform.machineIdentifier.dataRepresentation.write(to: identity)
        }
